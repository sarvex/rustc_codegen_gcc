...
def clone_repository(repo_name, path, repo_url, sub_paths=None):
    if os.path.exists(path):
        while True:
            choice = input(
                f"There is already a `{path}` folder, do you want to update it? [y/N]"
            )
...
def update_intrinsics(llvm_path, llvmint, llvmint2):
    ...
    archs = sorted(intrinsics)
    output_file = os.path.join(
        os.path.dirname(os.path.abspath(__file__)),
        "../src/intrinsic/archs.rs",
    )
    print(f"Updating content of `{output_file}`...")
        ...
        for arch in archs:
            ...
            for entry in intrinsics[arch]:
                if entry[2] == True: # if it is a duplicate
                    out.write(f'    // [DUPLICATE]: "{entry[0]}" => "{entry[1]}",\n')
                elif "_round_mask" in entry[1]:
                    out.write(f'    // [INVALID CONVERSION]: "{entry[0]}" => "{entry[1]}",\n')
                else:
